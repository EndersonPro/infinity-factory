...
fn dash_or_hls_only_has_no_progressive_but_sets_dash_flag() {
    let dash_only = r#"<script type="application/json" data-sjs>{"require":[["ScheduledServerJS","handle",[["VideoConfig",[],{"video_data":{"video_id":"1","playable_url_dash":"https://video.example.invalid/dash.mpd"}}]]]]}</script>"#;
    ...
    let mixed = r#"<script type="application/json" data-sjs>{"require":[["ScheduledServerJS","handle",[["VideoConfig",[],{"video_data":{"video_id":"1","playable_url":"https://video.example.invalid/sd.mp4","playable_url_dash":"https://video.example.invalid/dash.mpd"}}]]]]}</script>"#;
...
fn reels_resolve_via_short_form_video_context() {
    let reel = r#"<script type="application/json" data-sjs>{"creation_story":{"short_form_video_context":{"playback_video":{"video_id":"1","playable_url":"https://video.example.invalid/reel.mp4"}}}}</script>"#;
...
fn malformed_data_sjs_block_maps_to_malformed_response() {
    let bad = r#"<script type="application/json" data-sjs>not-json</script>"#;
...
fn absent_data_sjs_or_absent_media_yield_no_media() {
    ...
    let no_media = r#"<script type="application/json" data-sjs>{"require":[["ScheduledServerJS","handle",[]]]}</script>"#;
    assert!(
...
fn rejects_http_and_userinfo_progressive_urls() {
    let html = r#"<script type="application/json" data-sjs>{"require":[["ScheduledServerJS","handle",[["VideoConfig",[],{"video_data":{"video_id":"1","playable_url":"http://video.example.invalid/sd.mp4","browser_native_hd_url":"https://user:pass@video.example.invalid/hd.mp4"}}]]]]}</script>"#;
    let media = extract_media(&extract_data_sjs(html))
        .expect("parses")
        .expect("media present");
...
fn rejects_non_https_thumbnail_but_keeps_progressive() {
    let html = r#"<script type="application/json" data-sjs>{"require":[["ScheduledServerJS","handle",[["VideoConfig",[],{"video_data":{"video_id":"1","playable_url":"https://video.example.invalid/sd.mp4","thumbnail":{"uri":"http://scontent.example.invalid/thumb.jpg"}}}]]]]}</script>"#;
    let media = extract_media(&extract_data_sjs(html))
        .expect("parses")
        .expect("media present");
    assert_eq!(media.progressive, ["https://video.example.invalid/sd.mp4"]);
